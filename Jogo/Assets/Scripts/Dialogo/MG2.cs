...
public class MG2 : MonoBehaviour
{
    ...
    private GerenciadorDeJogoMG2 gerenciadorScript; // Referência ao script GerenciadorDeJogoMG1


    // Start is called before the first frame update
    void Start()
    {
        gerenciadorScript = gerenciadorObj.GetComponent<GerenciadorDeJogoMG2>(); // Obtém o script GerenciadorDeJogoMG1
    ...
    void Update()
    {
        ...
        {
            gerenciadorScript.fimDaMensagem2 = true;
        }
    }
...
{
    if (!dialogoFinalIniciado && index == 2) // Inicia o diálogo final apenas uma vez
    {
...
{
    if (index == 2 && (Input.GetKeyDown(KeyCode.Space) || Input.GetMouseButtonDown(0))) // Verifica se o índice é igual a 3
    {
        gameObject.SetActive(false); // Desativa o objeto
        gerenciadorScript.jogoIniciou = true; // Define a variável jogoIniciou como verdadeir
    }
}
